...
Types.gpcData = {
    ...
    toObject(object, debug = {}) {
        // object is byte array like [1,2,3,...]
        if (debug.use_default && object === undefined) {
            return {};
        }
        v.required(object);
        var b = Buffer.from(object, "binary");
        var bpos = 0;

        var codeHash = [];
        for (var i = 0; i < 20; i++) {
            codeHash.push(b.readUInt8(bpos));
            bpos += 1;
        }
        var codeSize = b.readInt32BE(bpos);
        bpos += 4;
        var codeBytes = this.newBuffer(codeSize);
        b.copy(codeBytes, 0, bpos, bpos + codeSize);
        bpos += codeSize;
        var apisCount = b.readInt32BE(bpos);
        bpos += 4;
        var abi = [];
        for (var i = 0; i < apisCount; i++) {
            var apiLength = b.readInt32BE(bpos);
            bpos += 4;
            var apiNameBytes = this.newBuffer(apiLength);
            b.copy(apiNameBytes, 0, bpos, bpos + apiLength);
            bpos += apiLength;
            abi.push(apiNameBytes.toString());
        }
        var offlineApisCount = b.readInt32BE(bpos);
        bpos += 4;
        var offlineAbi = [];
        for (var i = 0; i < offlineApisCount; i++) {
            var apiLength = b.readInt32BE(bpos);
            bpos += 4;
            var apiNameBytes = this.newBuffer(apiLength);
            b.copy(apiNameBytes, 0, bpos, bpos + apiLength);
            bpos += apiLength;
            offlineAbi.push(apiNameBytes.toString());
        }
        var eventsCount = b.readInt32BE(bpos);
        bpos += 4;
        var events = [];
        for (var i = 0; i < eventsCount; i++) {
            var eventLength = b.readInt32BE(bpos);
            bpos += 4;
            var eventNameBytes = this.newBuffer(eventLength);
            b.copy(eventNameBytes, 0, bpos, bpos + eventLength);
            bpos += eventLength;
            events.push(eventNameBytes.toString());
        }
        var storagePropertiesCount = b.readInt32BE(bpos);
        bpos += 4;
        var storageProperties = [];
        for (var i = 0; i < storagePropertiesCount; i++) {
            var nameLength = b.readInt32BE(bpos);
            bpos += 4;
            var nameBytes = this.newBuffer(nameLength);
            b.copy(nameBytes, 0, bpos, bpos + nameLength);
            b = b.slice(nameLength);
            var name = nameBytes.toString();
            var storageType = b.readInt32BE(bpos);
            bpos += 4;
            storageProperties.push([name, storageType]);
        }
        var gpcObject = {
            code_hash: Buffer.from(codeHash, "binary").toString("hex"),
            code: codeBytes.toJSON().data,
            abi: abi,
            offline_abi: offlineAbi,
            events: events,
...
Types.codeBytes = function(size) {
    return {
        ...
        appendByteBuffer(b, object) {
            v.required(object);
            if (typeof object === "string") object = Buffer.from(object, "hex");
            else object = Buffer.from(object);
...
Types.tuple = function() {
    var operationsCount = arguments.length;
    var stOperations = arguments;
    return {
        fromByteBuffer(b) {
            ...
            for (
                ...
                var st_operation = stOperations[i];
                result.push(st_operation.fromByteBuffer(b)); // TODO
            }
            return result;
        },
        appendByteBuffer(b, object) {
            v.required(object);
            b.writeVarint32(object.length);
            if (object.length > operationsCount) {
                throw new Error("invalid tuple arguments");
            }
...
Types.time_point = {
    ...
    appendByteBuffer(b, object) {
        if (typeof object !== "number")
            object = Types.time_point.fromObject(object);
    ...
    toObject(object, debug = {}) {
        ...
        v.require_range(0, 0xffffffffffffffff, int, `int64 ${object}`);
        return new Date(int).toISOString().split(".")[0];
    }
};
...
Types.address = {
    _to_address(object) {
        ...
        return Address.fromString(object, ChainConfig.address_prefix);
    },
    fromByteBuffer(b) {
        const version = b.readUint8();
        return new Address(fp.ripemd160(b), version);
    },
    appendByteBuffer(b, object) {
        var addr = Types.address._to_address(object);
        b.writeByte(addr.version);
        fp.ripemd160(b, addr.getBufferWithoutVersion());
        return b;
    },
    fromObject(object) {
