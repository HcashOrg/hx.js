...
helper.getEmptyAddress = function() {
    if (ChainConfig.address_prefix === "HXT") {
        return "HXTNKuyBkoGdZZSLyPbJEetheRhMjezkaXk2J";
    }
...
helper.listCitizens = function(apiInstance, prefix, limit) {
    return new NodeClient(apiInstance).listCitizens(prefix, limit);
};

// 查询citizens数量
helper.getCitizensCount = function(apiInstance) {
    return new NodeClient(apiInstance).getCitizensCount();
};

// 查询citizen信息
helper.getCitizen = function(apiInstance, citizenIdOrAccountName) {
    return new NodeClient(apiInstance).getCitizen(citizenIdOrAccountName);
};
