...
helper.new_transfer_operation = function(
    ...
    if (memo) {
        if (typeof memo === "string") {
            memo = helper.hex_to_bytes(helper.utf8ToHex(memo));
        }
        memoObject = {
...
// 查询citizen列表
helper.listCitizens = function(apiInstance, prefix, limit) {
    return apiInstance.db_api().exec("lookup_miner_accounts", [prefix, limit]);
};

// 查询citizens数量
helper.getCitizensCount = function(apiInstance) {
    return apiInstance.db_api().exec("get_miner_count", []);
};

// 查询citizen信息
helper.getCitizen = function(apiInstance, citizenIdOrAccountName) {
    return apiInstance.db_api().exec("get_miner", [citizenIdOrAccountName]);
};
...
helper.getAccountByName = function(apiInstance, accountName) {
    return apiInstance.db_api().exec("get_account_by_name", [accountName]);
};
...
helper.getAccountLockBalances = function(apiInstance, accountName) {
    return apiInstance.db_api().exec("get_account_lock_balance", [accountName]);
};
